...
import {LayoutSideContent, Bell, Envelope, Gear, House, Person, Rocket, CirclePlus, Bars} from "@gravity-ui/icons";
...
export function DashboardSidebar() {
  ...
  const founderNav = [
    ...
    {icon: Bars,href:'/dashboard/founder/opportunities', label: "My Opportunities"},
    {icon: CirclePlus,href:'/dashboard/founder/opportunities/new', label: "Create Opportunity"},
  ...
  const collaboratorNav = [
    ...
    {icon: Rocket,href:'/dashboard/collaborator/applications', label: "My Applications"},
    {icon: CirclePlus,href:'/dashboard/collaborator/opportunities', label: "Browse Opportunities"},
  ...
  const adminNav=[
    ...
    {icon: CirclePlus,href:'/dashboard/admin/startups', label: "Manage Startups"},
    {icon: CirclePlus,href:'/dashboard/admin/transactions', label: "Transactions"},
    {icon: CirclePlus,href:'/dashboard/admin/profile', label: "Profile"},
  ];

  const navItems =
    role === "admin" ? adminNav :
    role === "collaborator" ? collaboratorNav :
    founderNav;
 ...
 const handleSignOut =async()=>{
  ...
  return (
    ...
    <aside className="hidden w-64 shrink-0 border-r border-default p-4 lg:flex lg:flex-col lg:justify-between h-screen sticky top-0 bg-[#FAFAFA]">
          ...
          {user ? (
            <>
              <Avatar size="sm">
                <Avatar.Image
                  alt={user.name}
                  src={user.image || "https://heroui-assets.nyc3.cdn.digitaloceanspaces.com/avatars/red.jpg"}
                />
                <Avatar.Fallback>{user.name?.slice(0, 2).toUpperCase()}</Avatar.Fallback>
              </Avatar>

              <div>
                <h4 className="text-sm font-bold text-[#131B3A] leading-tight truncate">{user.name}</h4>
                <p className={`text-[10px] font-bold uppercase tracking-wider ${role === "admin"? "text-[#FF6B35]" : role === "founder" ? "text-[#131B3A]" : "text-indigo-600"}`}>{role}</p>
              </div>
            </>
          ) : (
            <>
              ...
              <div>
                <h4 className="text-sm font-semibold text-[#131B3A]">Loading...</h4>
      ...
      text-[#FAFAFA] mt-6 ml-2" variant="secondary">
        <LayoutSideContent />
