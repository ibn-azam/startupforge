    ...
    try {
      ...
      setStartups((currentStartups) =>
        currentStartups.filter(
          (currentStartup) => currentStartup._id !== startup._id,
        ),
      );
    } catch (removeError) {
    ...
    <div className="flex min-h-full flex-col gap-8 p-6 lg:p-8">
      <div>
        ...
        <p className="mt-2 text-sm text-gray-500">
          Review submitted startups, approve trusted listings, or remove them
          from the platform.
        </p>
      </div>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              {startups.length} startup{startups.length === 1 ? "" : "s"} from
              the database
            </p>
          </div>
          <Input
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex min-w-0 items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <h3 className="truncate font-semibold text-[#131B3A]">
                          {startup.name || "Unnamed startup"}
                        </h3>
                        <p className="mt-1 truncate text-sm text-gray-500">
                          {startup.founderEmail || "Founder not specified"}
                        </p>
                      </div>
                    </div>
                    <span
                      className={`shrink-0 rounded-full px-3 py-1 text-xs font-semibold capitalize ${statusStyles[startup.status] || "bg-gray-100 text-gray-700"}`}
                    >
                  ...
                  <div className="mt-5 flex flex-wrap gap-2 border-t border-gray-100 pt-4 text-xs font-medium">
                    {startup.industry && (
                      <span className="rounded-md bg-[#FF6B35]/10 px-2.5 py-1 text-[#FF6B35]">
                        {startup.industry}
                      </span>
                    )}
