...
const SignUpPage = () => {
    ...
    const onSubmit = async(e)=>{
        ...
        if (imageFile && imageFile.size > 0) {
            setIsUploading(true)
            setUploadError(null)
            const uploadResult = await uploadImageToImgbb(formData);
            ...
            <Card className="w-full max-w-md sm:max-w-lg md:max-w-xl p-5 sm:p-8 shadow-sm rounded-2xl">
                ...
                <Form className="flex flex-col gap-4 sm:gap-5" onSubmit={onSubmit}>
                    ...
                    >
                        ...
                        <Input accept="image/*" className="w-full" onChange={handleImageChange} />
                    ...
                    <div className="flex flex-col gap-3 my-2">
                        <Button type="submit" isDisabled={isUploading} className="w-full text-[#FAFAFA] bg-[#FF6B35] font-bold text-[16px]">
                            
                        {isUploading ? "Uploading..." : "Create"}
                        </Button>
                        <Button type="reset" variant="secondary" className="w-full text-[#131B3A] font-semibold text-[16px]">
                            Reset
                        </Button>
                    </div>
                </Form>
